...
void viktor_init_crazy(void)
{
	...
	for (cpu = 0; cpu < CONFIG_NR_CPUS; cpu++) {
		...
		atomic_set(&vrcu_data[cpu].idle_ct_total, 0);
...
void print_vrcu_data(int cpu)
{
	...
	int idle_ct_total = atomic_read(vrcu_data[cpu].idle_ct_total);
	...
	pr_err("vrcu_data\n{is_crazy=%d, loop_iter=%d, loop_total=%d, idle_ct=%d, idle_warn=%d\nidle_ct_total=%d, irq=%d, irq_warn=%d, irq_total=%d }\n", is_crazy, loop_iter, loop_total, idle_ct, idle_warn, idle_ct_total, irq, irq_warn, irq_total);
}
