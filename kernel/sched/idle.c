...
void viktor_init_crazy(void)
{
	...
	for (cpu = 0; cpu < CONFIG_NR_CPUS; cpu++) {
		...
		atomic_long_set(&vrcu_data[cpu].stop_tick, 0);
		atomic_long_set(&vrcu_data[cpu].stopped_at, 0);
...
void print_vrcu_data(int cpu, const char *append)
{
	...
	long stop_tick = atomic_long_read(&vrcu_data[cpu].stop_tick);
	long stopped_at = atomic_long_read(&vrcu_data[cpu].stopped_at);
	...
	if (append == NULL)
		pr_err("vrcu_data jiffies=%lu cpu=%d {is_crazy=%d, loop_iter=%d, loop_total=%d, idle_ct=%d, idle_warn=%d idle_total=%d, irq=%d, irq_warn=%d, irq_total=%d stop_inf=%d stop_tick=%ld stopped_at=%ld last_sched_clock=%ld mod_stop=%d mod_value=%ld bc_exp_true=%ld bc_exp_false=%ld bc_exp_sent=%ld idle_exit=%ld idle_update_tick=%ld restart_sched_tick=%ld}\n", j, cpu, is_crazy, loop_iter, loop_total, idle_ct, idle_warn, idle_total, irq, irq_warn, irq_total, stop_inf, stop_tick, stopped_at, last_sched_clock, mod_stop, mod_value, bc_expired_true, bc_expired_false, bc_expired_sent, idle_exit, idle_update_tick, restart_sched_tick);
	else
		pr_err("vrcu_data%s jiffies=%lu cpu=%d {is_crazy=%d, loop_iter=%d, loop_total=%d, idle_ct=%d, idle_warn=%d idle_total=%d, irq=%d, irq_warn=%d, irq_total=%d stop_inf=%d stop_tick=%ld stopped_at=%ld last_sched_clock=%ld mod_stop=%d mod_value=%ld bc_exp_true=%ld bc_exp_false=%ld bc_exp_sent=%ld idle_exit=%ld idle_update_tick=%ld restart_sched_tick=%ld}\n", append, j, cpu, is_crazy, loop_iter, loop_total, idle_ct, idle_warn, idle_total, irq, irq_warn, irq_total, stop_inf, stop_tick, stopped_at, last_sched_clock, mod_stop, mod_value, bc_expired_true, bc_expired_false, bc_expired_sent, idle_exit, idle_update_tick, restart_sched_tick);
}
...
void vrcu_debug_stop_tick(long j, ktime_t expires)
{
	...
	atomic_long_set(&vrcu_data[cpu].stopped_at, (long) jiffies);
	j += (long) jiffies;
	if (expires != KTIME_MAX)
