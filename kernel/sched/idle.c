...
void viktor_init_crazy(void)
{
	...
	for (cpu = 0; cpu < CONFIG_NR_CPUS; cpu++) {
		...
		atomic_set(&vrcu_data[cpu].stop_neg, 0);
		atomic_long_set(&vrcu_data[cpu].stop_tick, 0);
...
void print_vrcu_data(int cpu, const char *append)
{
	...
	int stop_neg = atomic_read(&vrcu_data[cpu].stop_neg);
	...
	if (append == NULL)
		pr_err("vrcu_data jiffies=%lu cpu=%d {is_crazy=%d, loop_iter=%d, loop_total=%d, idle_ct=%d, idle_warn=%d idle_total=%d, irq=%d, irq_warn=%d, irq_total=%d stop_neg=%d stop_tick=%ld last_sched_clock=%ld mod_stop=%d mod_value=%ld}\n", j, cpu, is_crazy, loop_iter, loop_total, idle_ct, idle_warn, idle_total, irq, irq_warn, irq_total, stop_neg, stop_tick, last_sched_clock, mod_stop, mod_value);
	else
		pr_err("vrcu_data%s jiffies=%lu cpu=%d {is_crazy=%d, loop_iter=%d, loop_total=%d, idle_ct=%d, idle_warn=%d idle_total=%d, irq=%d, irq_warn=%d, irq_total=%d stop_neg=%d stop_tick=%ld last_sched_clock=%ld mod_stop=%d mod_value=%ld}\n", append, j, cpu, is_crazy, loop_iter, loop_total, idle_ct, idle_warn, idle_total, irq, irq_warn, irq_total, stop_neg, stop_tick, last_sched_clock, mod_stop, mod_value);
}
...
void vrcu_debug_stop_tick(long j, ktime_t expires, ktime_t k)
{
	int cpu = smp_processor_id();

	if (j < 0) {
		atomic_inc(&vrcu_data[cpu].stop_neg);
	} else if (j >= 2500)
		pr_err("%s() cpu=%d j=%ld expires=%lld k=%lld\n", __func__, cpu, j, expires, k);
	else {
