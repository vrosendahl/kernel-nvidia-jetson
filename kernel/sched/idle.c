...
void print_vrcu_data(int cpu, const char *append)
{
	...
	if (append == NULL)
		pr_err("vrcu_data jiffies=%lu cpu=%d {is_crazy=%d, loop_iter=%d, loop_total=%d, idle_ct=%d, idle_warn=%d idle_total=%d, irq=%d, irq_warn=%d, irq_total=%d stop_tick=%ld last_sched_clock=%ld mod_stop=%d mod_value=%ld}\n", j, cpu, is_crazy, loop_iter, loop_total, idle_ct, idle_warn, idle_total, irq, irq_warn, irq_total, stop_tick, last_sched_clock, mod_stop, mod_value);
	else
		pr_err("vrcu_data%s jiffies=%lu cpu=%d {is_crazy=%d, loop_iter=%d, loop_total=%d, idle_ct=%d, idle_warn=%d idle_total=%d, irq=%d, irq_warn=%d, irq_total=%d stop_tick=%ld last_sched_clock=%ld mod_stop=%d mod_value=%ld}\n", append, j, cpu, is_crazy, loop_iter, loop_total, idle_ct, idle_warn, idle_total, irq, irq_warn, irq_total, stop_tick, last_sched_clock, mod_stop, mod_value);
}
...
void vrcu_debug_stop_tick(unsigned long j)
{
	int cpu = smp_processor_id();
